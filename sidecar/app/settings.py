...
Env vars (parsed by load_settings, then pushed into edgartools by apply_settings):
    ...
    EDGAR_RATE_LIMIT_PER_SEC  default 8    SEC request throttle; pinned into the env before edgar
                                           imports (see app/__init__.py) so HTTP_MGR builds at this rate
...
from edgar import set_identity, set_local_storage_path, use_local_storage
...
def apply_settings(settings: Settings) -> None:
    ...
    # The SEC throttle (settings.rate_limit_per_sec) is pinned into EDGAR_RATE_LIMIT_PER_SEC by
    # app/__init__.py BEFORE edgartools is imported, so HTTP_MGR already built its limiter at that
    # rate -- nothing to reconfigure here.
