...
def test_financials_ifrs_foreign_filer(client: TestClient, golden) -> None:
    ...
    # get_operating_cash_flow resolves for this filer while operating_income/capex/
    # shares return None - their getter patterns do not match this filer's IFRS
    # concepts and labels (the 'Operating profit' row above is on the statement)
...
def test_financial_metrics(client: TestClient, golden) -> None:
    ...
    assert response.status_code == 200
    body = response.json()
    # operating_cash_flow/free_cash_flow are None because get_operating_cash_flow
    # matches label regexes against the rendered statement and AAPL's rendered label
    # 'Cash generated by operating activities' matches none of its patterns. The
    # actual OCF (111482000000) ships in the statements endpoint above.
    assert body == {
        "cik": "0000320193",
        "company": "Apple Inc.",
        "form": "10-K",
...
def test_financials_ttm_concept_and_as_of(client: TestClient, golden) -> None:
    ...
    # periods carry each fact's tagged fiscal_year (calculate_ttm builds them from
    # q.fiscal_year); SEC tags comparative-period facts with the reporting filing's
    # fiscal year, so this historical window shows shifted year labels for quarters
    # ending in FY24/FY25. calculate_ttm_trend derives label years from period_end
    # instead (GH #793).
