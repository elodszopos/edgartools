...
def test_financials_ifrs_foreign_filer(client: TestClient, golden) -> None:
    ...
    assert metrics == {
        ...
        "stockholders_equity": 11205000000.0,
...
def test_financials_multi_annual(client: TestClient, golden) -> None:
    ...
    assert body["period"] == "annual"
    assert body["amendments"] is False
    # all 3 10-Ks parse cleanly -> each flagged parsed=true, parse_error=null (honest
    # provenance, #2); a filing whose XBRL failed would stay here flagged parsed=false
    assert body["filings"] == [
        {
            "form": "10-K",
            "accession_number": "0000320193-25-000079",
