...
- NO fixture size limits (user directive 2026-06-12): pick whatever filings make the BEST tests - never trim, defer, or substitute a test case because of cassette/golden size. Multi-GB fixtures are acceptable. Interaction-count and rate rules above still apply.
...
U12 | done | /tickers serves the full company_tickers_exchange map (cik/ticker/name/exchange) with uniform paging; defaults (page_size 20000) return all ~10k rows in one request. exchange null for unlisted/OTC entries via to_str empty->None; rows keep SEC file order (market-cap-ish, NVDA first). Ticker required (converter raises on blank - map rows always carry one); cik padded. Ground truth: AAPL row exactly {0000320193, AAPL, Apple Inc., Nasdaq}, JPM on NYSE, total>5000. Cassette 566KB (1 interaction). P1 COMPLETE. Gates: 75 pytest, pyright 0, drift+bun 16, eslint green.
U20 | done | /filing/{accession} via get_by_accession_number_enriched (quarterly-index scan collects ALL entity rows; current-year accessions missing from indexes fall back to getcurrent feed which paginates the WHOLE feed before not-found -> slow 404, comment in router). Envelope = index entities + full SEC-HEADER model (filers/reporting_owners/issuer/subject_companies w/ addresses, former names, filing values) + primary docs + URLs + data=null; FilingEnvelope moved common.py -> models/filing.py. NEW serialize.eastern_naive_to_utc: SEC-HEADER ACCEPTANCE-DATETIME is naive US-Eastern (Carnival 09:00:22 EST -> 14:00:22Z). ITEM INFORMATION re-parsed from raw header text (filing_metadata comma-joins repeated tags - lossy); reporting owner serves raw conformed name (Owner.name lazily fetches Entity per owner - cross-entity call); header parser appends None for empty REPORTING-OWNER blocks (skipped). Fixtures: Walgreens 8-K (single-entity, 3 items), Carnival joint 8-K (2 filers + former names), NVIDIA Form 4 (issuer+owner branches), 404 via 1995 accession (keeps not-found scan in small old indexes). Cassette 12.4MB / 8 interactions. Gates: 82 pytest, pyright 0, drift+bun 21, eslint green.
U21 | done | /content fmt=markdown(default)|text|html + page_breaks (markdown-only, else 422); /sections fmt=text|markdown via modern edgar.documents.HTMLParser(ParserConfig(form=)) NOT deprecated Filing.sections()/htmltools chunker (removal v6.0) - sections carry name/title/part/item/detection_method/confidence, ordered by start_offset; non-HTML primaries (rendered Form 4 XML) -> total=0 not error. NEW cassette pattern: vcr_cassette_name fixture override makes all module tests share filing_p2_fixtures.yaml (git mv from U20 cassette); appending needs one --vcr-record=new_episodes run (once-mode forbids); in-session disk cache keeps each test standalone. Recording surprises (all deterministic, replay offline): sgml.html() returns None for WBA+Anixa primaries -> Filing.html() falls back to homepage -index.html + direct .htm download (2 extra interactions each); Ownership.to_html() fetches data.sec.gov submissions per owner (NVDA). Ground truth: Anixa 10-K 22 TOC sections (part_i_item_1 conf 0.95 .. part_iv_item_16), WBA 8-K 3 pattern sections (item_202/701/901 conf 0.7, text renders U+2009 thin space after "Item"), NVDA Form 4 sections=0. Cassette 19.8MB / 14 interactions (+6). Gates: 84 pytest, pyright 0, drift+bun 29, eslint green.
U22 | done | /attachments lists every SGML submission document (documents-then-data_files order via the SGML XML latch, is_primary/is_binary flags, display_description merges FilingSummary report names + standard exhibit text, purpose for R-files); /attachments/{seq} fmt=text(default)|raw|markdown extracted from the in-memory SGML - ZERO new SEC interactions (module shares filing_p2_fixtures cassette; all content already inside the recorded .txt). raw on binary or uudecoded-bytes -> 422 carrying the download url; text of binary -> null content; missing seq -> 404 naming seq+accession. HARNESS BUG FIXED: vcrpy applies before_record_response on cassette _load too (Cassette._load -> append) - the U02 budget guard counted REPLAYS ("fires only while recording" comment was wrong) and tripped >60 once two modules loaded the 14-interaction shared cassette; guard now frame-walks and skips _load-originated calls, record-path appends still counted. Ground truth: WBA 16 attachments vs header document_count 17 (SGML sequence gaps 9/14/16), EX-99.1 text starts "Exhibit 99.1\nWalgreens Boots Alliance Reports Fiscal 2025 First Quarter Results"; Anixa 63 = 7 documents + 56 data files (R2.htm purpose "Consolidated Balance Sheets", report text renders the statement table from in-filing FilingSummary, no fetch); NVDA Form 4 single XML doc (raw==text passthrough, markdown null). Gates: 88 pytest, pyright 0, drift+bun 39, eslint green.

U30 | done | /company/{id} (CIK or ticker) profile from the SEC submissions store JSON ONLY (parse_entity_submissions; expensive cross-filing/facts properties excluded by design: filer_type, latest_tenk/q, public_float etc. belong to dedicated endpoints); is_foreign via is_foreign_company(state) only when a state code exists else null (library fallback would fetch filings); display_name reverses individuals' "Last First". /company/{id}/submissions pages the full filings table (trigger_full_load pulls pagination files; AAPL 2234 rows back to 1994), form filter client-side, items CSV->list, isXBRL/isInlineXBRL->bool|null. NEW serialize.utc_naive_to_utc: acceptanceDateTime is UTC-Z at source but pyarrow round-trips drop tz. CompanyNotFoundError->404 (str(exc) carries "did you mean" ticker suggestions); dead CIK->404 "no entity at SEC with CIK". HARNESS TIME BOMB FIXED: the disk HTTP cache judges freshness from the RECORDED Date header, so any cassette older than its URL's TTL (submissions/tickers 30s per #471, indexes 30min) stops deduping repeat in-test requests -> play-once exhaustion; broke test_filings once its cassette aged past 30min (was green at U22 only because the cassette was fresh). Fix: @pytest.mark.vcr(allow_playback_repeats=True) on re-requesting tests (kwarg is use_cassette-only, MUST flow via marker - in vcr_config it TypeErrors the VCR ctor). Ground truth: AAPL profile exact-dict (FYE 0926, EIN 942404110, 3 former names), Carnival D0/Bermuda is_foreign=true (submissions store=current registration; 1992 SGML header says R1/Panama), Ajay Puri individual (no business address, owner-exists flag, reversed display name); 10-K total=32 latest 0000320193-25-000079; 8-K items ["2.02","9.01"]; deep page start=2230 -> 4 rows has_more=false. Cassette company_p3_fixtures 3.0MB / 10 interactions. Gates: 101 pytest, pyright 0, drift+bun 49, eslint green.

U31 | done | /company/{id}/financials + /financials/metrics. Statement shape: Statement.to_dataframe(presentation=False, include_unit, include_point_in_time; view=SUMMARY or DETAILED via dimensions flag) unpivoted into typed values[] records keyed by XBRL period keys; the library exposes NO public period->column mapping, so the converter mirrors the (FY)/(Qn)/(YTD) column-naming logic from statements.py and maps BY NAME - unmapped columns raise RuntimeError (loud drift detection); transition-period column collisions (#582) keep-first via setdefault. Form selection mirrors get_financials chains (10-K/20-F/40-F, 10-Q/6-K) but in the router so the response carries provenance; filing-without-XBRL and exhausted-chain -> distinct 404s. Getter behavior traced code-level: get_operating_cash_flow has NO concept-based lookup (only label regexes; AAPL's rendered label 'Cash generated by operating activities' on us-gaap_NetCashProvidedByUsedInOperatingActivities matches none of its 5 patterns -> OCF/FCF null while actual OCF 111482000000 ships in statements); inverse on IFRS: INFY metrics resolve OCF but operating_income/capex/shares null despite ifrs-full_ProfitLossFromOperatingActivities 4071000000 on the statement; bd CLI not installed -> behavior recorded here. USER DIRECTIVE: fixture size budget ELIMINATED (plan rule rewritten) - INFY 20-F (98MB SGML) re-recorded via --vcr-record=new_episodes (pytest-vcr flag; pytest-recording's --record-mode does not exist here), full IFRS statement+metrics coverage in-unit instead of deferred. Ground truth: AAPL 10-K 0000320193-25-000079 (Net sales 416161000000 credit/1.0/usd, 18/37/35/11/13/36 records, cover ships text facts '10-K'/'--09-27' as wire strings, OCF row standard_concept NetCashFromOperatingActivities), raw+dimensions 47 IS records/29 dims (iPhone 209586000000 srt:ProductOrServiceAxis), 10-Q 0000320193-26-000013 Quarterly+Semi-Annual labels (Q2 FY26 net sales 111184000000), Realty Income lease income 5437332000, INFY 20-F u_usd unit / Revenues 19277000000, Puri 404 'no annual filing (10-K/20-F/40-F)...', metrics exact dicts incl. documented nulls. Cassette financials_p3_fixtures 148MB / 11 interactions; 7 goldens (5 statements + 2 metrics). Gates: 109 pytest, pyright 0, drift+bun 60, eslint green.

U32 | done | /financials/multi + /financials/ttm. Multi: XBRLS.from_filings over head(n) of the U31 form chains (n 2-8, default 4; provenance filings list = what was handed to the stitcher, newest first - XBRLS silently drops filings whose XBRL parse fails, so periods reveal actual coverage); StitchedStatement.statement_data consumed DIRECTLY (values/preferred_signs keyed by XBRL period_id - none of U31's column-name mirroring needed); StatementView.DETAILED/SUMMARY via dimensions flag. One column per filing by design (GH #780): quarterly stitches mix YTD and discrete-quarter durations (AAPL labels 'Q2 YTD Mar 28, 2026'/'Q1 Dec 27, 2025'/'Q3 YTD Jun 28, 2025'). DETERMINISM FIX (library, user-approved, own commit): stitched record order varied across interpreter hash seeds - goldens flipped AccountsPayableCurrent<->AccountsReceivableNetCurrent between runs; root cause ordering.py _extract_all_concepts returned a set whose iteration order seeded every downstream tie-break (dict insertion, +0.1 position cascades, max() ties); now first-appearance-ordered List[str]. Evidence: PYTHONHASHSEED 0/1/7/42 full-file sweeps byte-identical vs goldens; fork test pre/post failure sets identical (env-caused fails both sides, /tmp/fork_tests_{baseline,after}.txt). TTM: company.get_facts() (None -> 404 'no XBRL facts') + get_ttm_revenue/net_income (convenience, errors -> null) and explicit ?concept= get_ttm (KeyError/ValueError -> 404 with library message); as_of ISO date or YYYY-QN pre-validated -> 422. TTM periods behavior (neutral, code-level): calculate_ttm builds period labels from each fact's tagged fiscal_year (calculator.py:187); SEC tags comparative-period facts with the REPORTING filing's fiscal year, so historical as_of windows show shifted year labels ((2025,Q3),(2024,Q4),(2026,Q1),(2026,Q2) for as_of 2025-Q1); calculate_ttm_trend derives label years from period_end instead (GH #793); value itself hand-verified: 39678+43879+58275+44867 = 186699M matches. USER DIRECTIVES this unit: BANNED from labeling library behavior defect/bug/finding anywhere (tests scrubbed, U31 log line above neutralized); every uv/bun/pytest command starts with explicit absolute cd (stray root .venv/uv.lock twice - user cleans manually); loop BROKEN by user pending these fixes - no self-rearm. Ground truth: AAPL multi n=3 exact filings/periods dicts, Total operating expenses is_total [62151,57467,54847]M, counts IS 18/BS 37/CF 34; O n=2 accessions+period keys; Puri 404; n=1/n=9 -> 422; TTM revenue 451442000000 / NI 122575000000 (windows (2025,Q3)..(2026,Q2), has_calculated_q4, derived-quarters warning), GrossProfit 216071000000, as_of 2025-Q1 -> 186699000000 @ 2025-03-29, bad concept/as_of/CIK exact error bodies. Cassette 211MB / 18 interactions (+7); 5 goldens (3 multi + 2 ttm). Gates: 115 pytest, pyright 0, drift+bun 69, eslint green.

REVIEW-WAVE | done | Full architecture review of P1-P3 surface produced 57 findings (5 Critical, 25 Significant, 27 Minor); user ordered ALL fixed, split into commit blocks. STORAGE: vcrpy retired for a URL-keyed fixture store (tests/sec_replay.py) - one decoded body per SEC URL under tests/fixtures/sec/<host>/<path>, replay is sole arbiter (miss = hard error, cache+throttle disabled), EDGAR_FIXTURE_RECORD=1 records with a 60-new-file/run budget guard; the 8 old cassette YAMLs are dead in HEAD (purge from unpushed history is a user-run git filter-repo, command in this Log's S-4 line). S-1 deps/infra (#38 numpy/pandas/pyarrow/orjson declared, #52 loop.sh honors pre-set SEC_EDGAR_USER_AGENT/EDGAR_IDENTITY, #40 compose verified already 8000:8000-aligned, #53 layout reconciled). S-2a wire schema pass + library stitched concept metadata (unit/balance/weight) and uniform preferred-sign guard (edgar f981c602). S-2b: library PUBLISHES df.attrs['period_columns'] = {column: (period_key,label)} at DataFrame build (edgar f0b6b198) so the converter stops mirroring statements.py column-naming - converter shrank 73 lines, goldens byte-identical by construction. S-3 routers/errors: canonical ErrorResponse{detail} + error_responses() helper (declaring 422 suppresses FastAPI HTTPValidationError; openapi now 0 HTTPValidationError refs), RequestValidationError flattened to 'query.page_size: <msg>'; shared resolve_entity_id (ValueError->422) + resolve_cik (ticker->find_cik->404), resolution moved BEFORE index download in /filings (unknown ticker -> 404 not silent empty page, #33); library get_filings honors amendments=False without form/filing_date (edgar 71514ee4, #12). #43 one canonical Address (6 nullable fields); #39 is_primary by sequence_number set not object identity; #44 obj_type+data both null until U40 typed forms (TODO(DEFERRED) anchored at converters/filing.py + models/filing.py).

S-4 | done | #11 docker: build from fork root (-f sidecar/Dockerfile, context ..) green; self-exiting container smoke (uvicorn bg + python urllib /health poll + self-kill, no docker stop) returned {"status":"ok","edgartools_version":"5.36.0","identity_set":true} 200 in 2s; image edgar-sidecar:latest 1.26GB; left container edgar-sidecar-smoke Exited(0) - user removes (no docker rm per rules). #17 financial fixture matrix recorded via store record mode (3 new filers, all ground truth hand-verified against raw iXBRL nonFraction tags, scale 6/0): JPM 10-K 0001628280-26-008131 FY2025 (bank unclassified balance sheet -> AssetsCurrent/LiabilitiesCurrent absent, current_ratio null; revenue null - JPM tags RevenuesNetOfInterestExpense 182447M not gross Revenues, and never OperatingIncomeLoss; operating_cash_flow GENUINELY -147782M, sign='-' in iXBRL, trading/lending asset growth; capex untagged -> FCF null; total_liabilities 4062462M, debt_to_assets 0.918); AMZN 10-K 0001018724-26-000004 FY2025 (NEVER tags us-gaap:Liabilities per XBRL frames API CY2025Q4I -> total_liabilities + debt_to_assets null, runs straight to LiabilitiesAndStockholdersEquity 818042M; capex 131819M -> FCF 7695M, current_ratio 1.051); FDCTECH CIK 1722731 amended FY2025 10-K THREE times (two same-day 06-08) - as-filed default serves original 0001493152-26-017945 stamped superseded_by the LATEST /A 0001493152-26-027771 (same-day tiebreak by higher accession), amendments=true serves the /A itself with superseded_by null; restatement is real (FY2024 comparative NetIncomeLoss flips -18781 -> +247544, Assets 63771196 -> 64051886). 6 new goldens (4 statements + 2 metrics), 9 new store files (~98MB; JPM .txt alone 62MB). Plan Log entry (this), review-wave entry above. Gates: 184 pytest, pyright 0, ruff format+check clean, eslint clean, bun 81 pass. git filter-repo (USER runs; never me) to drop the dead sidecar cassettes from the 90 unpushed commits: from a fresh isolated clone `git clone --no-hardlinks <repo> /tmp/edgartools-clean && cd /tmp/edgartools-clean && git filter-repo --invert-paths --path sidecar/tests/cassettes/` (fresh clone -> no --force; --dry-run first; verify `git rev-list --objects --all | grep sidecar/tests/cassettes` empty + library tests/cassettes still 117 + HEAD tree unchanged; re-add origin, push is fast-forward since origin/main is an unchanged ancestor).

S-5 | done | Remaining review findings (sidecar-only; the library already shipped the providers). #51 (Minor): lookup_company 404 detail now pads the CIK (deps.py pad_cik). The existing not-found test used CIK 9999999999 (already 10 digits -> padded==bare) so the bug was invisible; added test_company_not_found_pads_short_cik with 999999999 -> "0999999999" (one live record, new store file CIK0999999999.json + meta; unassigned-high-range like the 10-nine sentinel, real SEC 404). #21 (Significant)+#50 (Minor): /financials/ttm convenience pair (revenue/net_income) was TTMMetricModel|null that CONFLATED concept-absent with compute-failed and 500'd on a malformed metric. Now a TTMConvenienceMetric wrapper {metric, unavailable_reason}, wired to the library's typed TTMConceptNotFoundError/TTMInsufficientDataError -> concept_absent / insufficient_quarters, plus sidecar 'malformed' for a non-finite/unitless metric (ttm_metric_model TypeError); explicit ?concept= path adds TypeError->502 (#50). as_of gate tightened to reject year outside 1900-2100 (422) so EntityFacts._parse_ttm_date can never raise a bare ValueError into the convenience helper (also upgrades the explicit path from a misclassified 404 to 422 on out-of-range years). New unit test test_ttm_convenience pins the 3-way classification + a drift guard (library TTMUnavailableReason values subset of the wire TTMUnavailable literal). Wire change regenerated openapi.json + zod.ts (orval) + 2 ttm goldens; TTMResponse.revenue/net_income are now always-present wrappers. #16 (Significant): VERIFIED FIXED, no change - test_financials_ttm_concept_and_as_of asserts the corrected consecutive labels [2024 Q3, 2024 Q4, 2025 Q1, 2025 Q2] (library 4df6da99); reran green. #2 (Critical): ASSESSED not implemented (user asked whether the fix is appropriate, not to fix it) - analyst verdict: listing raw INPUT filings in MultiFinancialsResponse.filings[] is phantom provenance; the honest fix wires xbrls.parse_outcomes (already exposed by the library: per-filing parsed/error, newest-first) into filings[] with a parsed flag, awaiting go-ahead. #5 (Critical) handled next as a history purge (S-4 runbook). Gates: 189 pytest, pyright 0 sidecar paths (edgar/ library baseline untouched), ruff clean, eslint clean, bun 81 pass.
